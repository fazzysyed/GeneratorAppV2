...
const ServiceImages = ({navigation, route}) => {
  ...
  useEffect(() => {
    ...
    fetch(
      `https://zacharydevworks.com/generator_app_backend/api/all-images?id=${id}`,
      requestOptions,
    )
  ...
  return (
    <Layout back={true} navigation={navigation}>
      <View
        ...
        {data.map((item, i) => {
          console.log(`https://zacharydevworks.com/generator_app_backend/api/storage/${item}`);
          if (i === index) {
            return (
              <Image
                source={{
                  uri: `https://zacharydevworks.com/generator_app_backend/api/storage/${item}`,
                }}
